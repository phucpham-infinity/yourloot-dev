      ...
      `,
        (err) => {
          if (err) {
            ...
            db.run(
              `ALTER TABLE orders ADD COLUMN statusCheckCount INTEGER DEFAULT 0`,
              (alterErr) => {
...
// Tăng số lần check status của order
export function incrementStatusCheckCount(userId, orderId) {
  return new Promise((resolve, reject) => {
    db.run(
      `
      UPDATE orders 
      SET statusCheckCount = statusCheckCount + 1
      WHERE userId = ? AND orderId = ?
    `,
      [userId, orderId],
      function (err) {
