...
import { levelsController } from '@/services/controller'
import { authController } from '@/services/controller/auth'
import { userController, UserProfile } from '@/services/controller/users'
import { useAuthStore, useDialogStore, useLevelStore } from '@/store'
...
const ProfileSetting = ({
  ...
  return (
    <div
      className={cn(
        'relative w-full h-full gap-5 p-5 lg:p-10 rounded-[20px] flex flex-col border-app-default',
        className,
      ...
      {isLoading ? (
        ...
        <>
          <div className="flex gap-5 items-center h-full ">
            <div css={styleAvatar} className="w-[128px] h-[128px]"></div>
            <div>
              <div className="w-[223px] h-32 flex-col justify-between items-start inline-flex">
                <div className="self-stretch h-[109px] flex-col justify-start items-start gap-3 flex">
          ...
          {!isEdit ? (
            <div className="flex gap-5 flex-col max-w-[667px] ">
              ...
              <div className="flex gap-2.5 flex-col">
                <div className=" text-[#c5c0d8] text-xs font-medium ">
                  {t('profile.cashback')} {levelStore?.cashBackValue}%
                </div>
                <ProgressBar
                  percentage={level?.content?.currentCashBackProgress || 0}
                  maxWidth={677}
                />
...
const UpdateProfie = ({
      ...
      },
      {
        onSuccess: () => {
          ...
          dialog.openBasicDialog({
            type: 'successful',
            meta: {
              title: t('profile.birthDateApplied'),
              description: t('profile.birthDateAdded'),
              button: (
                <div className={'w-full'}>
                  <CustomButton
                    onClick={() => {
  ...
  const { t } = useTranslation()
  return (
    <div className="flex gap-5 items-start justify-between flex-col  z-10 mt-10">
      ...
      <div className="flex justify-end items-center gap-2.5 w-full mt-5">
